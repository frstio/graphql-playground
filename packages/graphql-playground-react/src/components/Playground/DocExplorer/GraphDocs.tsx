...
> {
	...
	componentWillReceiveProps(nextProps: SideTabContentProps & StateFromProps) {
		...
		if (
			this.props.docs.navStack.length !==
				nextProps.docs.navStack.length ||
	...
	render() {
		...
		if (schema === undefined) {
			...
			emptySchema = (
				<ErrorContainer>{'No Schema Available'}</ErrorContainer>
			)
		}
			...
			>
				...
				{navStack.map((stack, index) => (
					<ColumnDoc
						key={index}
						width={
							this.state.widthMap[stack.field.path] || columnWidth
						}
					>
						<FieldDoc
							schema={schema}
