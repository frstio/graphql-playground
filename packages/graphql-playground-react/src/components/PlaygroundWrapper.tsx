...
> {
	...
	mapPropsToState(props: PlaygroundWrapperProps): State {
		...
		const { activeEnv, projectName } = this.getInitialActiveEnv(
			props.config,
		)
					...
					<App>
						{this.props.config && this.state.activeEnv && (
							<ProjectsSideNav
								...
								folderName={
									this.props.folderName || 'GraphQL App'
								}
								theme={theme}
						...
						<Playground
							...
							subscriptionEndpoint={
								this.state.subscriptionEndpoint
							}
	...
	private async updateSubscriptionsUrl() {
		const candidates = this.getSubscriptionsUrlCandidated(
			this.state.endpoint,
		)
	...
	private getSubscriptionsUrlCandidated(endpoint): string[] {
		...
		if (endpoint.includes('graph.cool')) {
			candidates.push(
				`wss://subscriptions.graph.cool/v1/${this.getProjectId(
					endpoint,
				)}`,
			)
		}
		if (endpoint.includes('/simple/v1/')) {
			...
			candidates.push(
				`ws://${host![1]}/subscriptions/v1/${this.getProjectId(
					endpoint,
				)}`,
			)
		}
