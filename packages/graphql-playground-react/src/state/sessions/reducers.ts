...
import { List, Map, merge, OrderedMap, Record } from 'immutable'
import { arrayMove } from 'react-sortable-hoc'
import { combineActions, handleActions } from 'redux-actions'
import { defaultQuery, getDefaultSession, ISession } from '../../constants'
import {
	closeQueryVariables,
	editEndpoint,
	editHeaders,
...
// tslint:disable
export class Session extends Record(getDefaultSession('')) implements ISession {
	// toJSON() {
...
const reducer = handleActions<SessionState, any>(
	{
		[combineActions(
			editQuery,
			editVariables,
		...
		START_QUERY: state => {
			return state
				...
				.setIn(['sessions', getSelectedSessionId(state), 'responseExtensions'], undefined)
		},
		...
		TOGGLE_TRACING: state => {
			const path = ['sessions', getSelectedSessionId(state), 'responseTracingOpen']
		...
		TOGGLE_VARIABLES: state => {
			const path = ['sessions', getSelectedSessionId(state), 'variableEditorOpen']
			return state.setIn(path, !state.getIn(path))
		},
		ADD_RESPONSE: (state, { payload: { response, sessionId } }) => {
			return state.updateIn(['sessions', sessionId, 'responses'], responses => responses.push(response))
		},
		...
		CLEAR_RESPONSES: state => {
			return state.setIn(['sessions', getSelectedSessionId(state), 'responses'], List())
		},
		FETCH_SCHEMA: state => {
			return state.setIn(['sessions', getSelectedSessionId(state), 'isReloadingSchema'], true)
		},
		...
		SCHEMA_FETCHING_SUCCESS: (state, { payload }) => {
			const newSessions = state.get('sessions').map((session: Session) => {
				if (session.endpoint === payload.endpoint) {
					...
					if (response instanceof ResponseRecord && session.responses!.size === 1 && response.isSchemaError) {
						data.responses = List([])
					}
		...
		SET_ENDPOINT_UNREACHABLE: (state, { payload }) => {
			const newSessions = state.get('sessions').map((session: Session, sessionId) => {
			...
			const newSessions = state.get('sessions').map((session: Session, sessionId) => {
				if (session.get('endpoint') === payload.endpoint) {
					...
					if (!responses || responses.size <= 1) {
						let response: ResponseRecord | null = session.responses ? session.responses!.first() : null
		...
		SET_SELECTED_SESSION_ID: (state, { payload: { sessionId } }) => state.set('selectedSessionId', sessionId),
		OPEN_SETTINGS_TAB: (state: any) => {
			let newState = state
			let settingsTab = state.sessions.find(value => value.get('isSettingsTab', false))
			if (!settingsTab) {
		...
		OPEN_CONFIG_TAB: state => {
			let newState = state
			let configTab = state.sessions.find(value => value.get('isConfigTab', false))
			if (!configTab) {
		...
		NEW_FILE_TAB: (state, { payload: { fileName, filePath, file } }) => {
			let newState = state
			let fileTab = state.sessions.find(value => value.get('name', '') === fileName)
			if (!fileTab) {
		...
		NEW_SESSION: (state, { payload: { reuseHeaders, endpoint } }) => {
			const currentSession: Session = state.sessions.first()
			...
			let session = makeSession(endpoint || currentSession.endpoint).merge(newSession)
			if (reuseHeaders) {
				const selectedSessionId = getSelectedSessionId(state)
				const currentSession = state.sessions.get(selectedSessionId) as Session
		...
		INJECT_HEADERS: (state, { payload: { headers, endpoint } }) => {
			...
			const headersString = typeof headers === 'string' ? headers : JSON.stringify(headers, null, 2)
			const selectedSessionId = getSelectedSessionId(state)
			let newState = state.set('headers', headersString)
			const currentSession = state.sessions.get(selectedSessionId)

			if (currentSession && currentSession.headers === headersString) {
				return newState
			}

			if (currentSession && currentSession.query === defaultQuery) {
		...
		SELECT_NEXT_TAB: (state: SessionState) => {
			const selectedSessionId = getSelectedSessionId(state)
			const count = state.sessions.size
			const keys = state.sessions.keySeq()
			const index = keys.indexOf(selectedSessionId)
			if (index + 1 < count) {
				return state.set('selectedSessionId', keys.get(index + 1) as string)
			}
			return state.set('selectedSessionId', keys.get(0) as string)
		},
		SELECT_PREV_TAB: state => {
			...
			return state.set('selectedSessionId', keys.get(count - 1) as string)
		},
		SELECT_TAB_INDEX: (state, { payload: { index } }) => {
			const keys = state.sessions.keySeq()
			return state.set('selectedSessionId', keys.get(index) as string)
		},
		...
		REORDER_TABS: (state: SessionState, { payload: { src, dest } }) => {
			const seq = state.sessions.toIndexedSeq()

			const indexes: number[] = []
			for (let i = 0; i < (seq.size as number); i++) indexes.push(i)
			const newIndexes = arrayMove(indexes, src, dest)

			let newSessions = OrderedMap<string, Session>()
			for (let i = 0; i < (seq.size as number); i++) {
				const ndx = newIndexes[i]
				const val = seq.get(ndx) as Session
				newSessions = newSessions.set(val.id, val)
			}
			return state.set('sessions', newSessions)
		},
		EDIT_SETTINGS: state => {
			return state.setIn(['sessions', getSelectedSessionId(state), 'changed'], true)
		},
		SAVE_SETTINGS: state => {
			return state.setIn(['sessions', getSelectedSessionId(state), 'changed'], false)
		},
		EDIT_CONFIG: state => {
			return state.setIn(['sessions', getSelectedSessionId(state), 'changed'], true)
		},
